...
The model’s performance was evaluated using standard multi-label classification metrics, including precision, recall, F1-score, and accuracy. Each crime type was treated as an independent binary classification task, and metrics were computed separately before averaging across all labels to obtain macro-level results. This ensured that each crime type contributed equally to the evaluation, regardless of how often it occurred. For the evaluation, we implemented a train test split, such that our training data consisted of data that was before or during 2015, and testing it on data from 2016-2017.

During evaluation, the model’s predictions were compared against ground-truth labels in the testing set. The evaluation script produced per-label and overall scores, allowing detailed comparisons across different crime types. Crimes with more consistent temporal patterns, such as theft and burglary, generally achieved higher recall, while lower-frequency crimes displayed slightly higher precision but lower recall. The F1-score provided a balanced view of overall performance across these trade-offs.

To better understand spatial and statistical patterns, several visuals were generated, including a sample crime density heatmap (see Figure~\ref{fig:image1}) and a metric correlation heatmap (see Figure~\ref{fig:image2}). The crime density map highlights spatial concentrations for one selected crime category as an illustrative example rather than a complete citywide representation. These visuals help contextualize model outputs and provide insight into crime clustering and label correlations. Please see these visuals at the end of the document after the team contributions section.

As a baseline, a simple frequency-based predictor was used to estimate weekly crime occurrences based on the previous week’s averages. The logistic regression model outperformed this baseline across all metrics, demonstrating its ability to generalize beyond recent history and identify broader temporal trends. Evaluation results were exported to \texttt{evaluation\_results.csv} for visualization and further analysis. Overall, these results establish a strong baseline and validate the effectiveness of the model’s rolling temporal features.
...
\textbf{Stanley:} Implemented the evaluation pipeline and data visualization heatmaps; wrote the Results and Evaluation section and integrated the visualizations into the report.
% 
...
\begin{figure*}[h!]
    ...
    \caption{Precision, recall, F1-score, and accuracy across different crime categories in Los Angeles.}
    \label{fig:image1}
\end{figure*}
...
\begin{figure*}[h!]
    ...
    \caption{Heatmap showing the geographic distribution and density of “Assault with a Deadly Weapon, Aggravated Assault” incidents in Los Angeles. Higher density areas (in red) indicate crime hotspots concentrated in specific city regions.}
    \label{fig:image2}
\end{figure*}

\begin{figure*}[t]
    \centering
    \vspace*{-2em} % optional: pulls image closer to the top margin
    \includegraphics[width=\textwidth, height=0.5\textheight, keepaspectratio]{images/image3.png}
    \caption{Summary of evaluation metrics for the logistic regression model.}
    \label{fig:image3}
    \vspace*{-1em} % optional: reduce space after figure
\end{figure*}
